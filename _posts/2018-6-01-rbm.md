...
Let's assume that the joint probability distribution of our MRF is strictly positive.  If this is the case, then we can apply the  [*Hammersley–Clifford Theorem*](https://en.wikipedia.org/wiki/Hammersley%E2%80%93Clifford_theorem) which tells us that a strictly positive distribution \\( p\\) can be represented as a MRF on a graph G iff it can be *factored over G*. \\( p\\) can be *factored over G* if

\\[
p = \frac{1}{Z}\Pi_C \psi_C
]\\
...
\\[
p = \frac{1}{Z}\Pi_C \psi_C = \frac{1}{Z}\exp\left(\sum_C\ln \psi_C\right).
]\\
